...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
            self,
            model_name="wan_models/Wan2.1-T2V-1.3B",
            timestep_shift=8.0,
            is_causal=False,
            local_attn_size=-1,
            sink_size=0,
        ...
        if is_causal:
            ...
            # self.model = CausalWanModel.from_pretrained(
            #     model_name, local_attn_size=local_attn_size, sink_size=sink_size, ignore_mismatched_sizes=True, low_cpu_mem_usage=False)  # 权重不匹配时使用
        else:
            self.model = WanModel.from_pretrained(model_name)
            if use_gan:
                print("[use_gan] activate")
    ...
    def forward(
        ...
        logits = None
        # X0 prediction
        if kv_cache is not None:  # generator
