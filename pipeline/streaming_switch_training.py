...
class StreamingSwitchTrainingPipeline(StreamingTrainingPipeline):
        ...
        *,
        ...
        for block_index, current_num_frames in enumerate(all_num_frames):
            ...
            for step_idx, current_timestep in enumerate(self.denoising_step_list):
                ...
                if not exit_flag:
                    ...
                    with torch.no_grad():
                        flow_pred, denoised_pred = self.generator(
                            ...
                            new_prompt_chunk_flag=block_index==0
                        )
                        if self.flow_cache is not None and dist.get_rank() == 0 and MY_VISUAL_DEBUG:
                            print("[SeqTrain-DMDSwitch] flow_pred update, flow_pred.sum is {}".format(flow_pred.sum()))
                    ...
                    if dist.get_rank() == 0 and MY_VISUAL_DEBUG:
                        print(f"[SeqTrain-DMDSwitch] Block {block_index} final step: enable_grad={enable_grad}, local_frame={local_start_frame}, denoise step={step_idx}")
                ...
                # 用热力图可视化flow_cache
                if self.flow_cache is not None and dist.get_rank() == 0 and MY_VISUAL_DEBUG and self.flow_cache['flow_pred'].sum() != 0:
                    flow_pred_pool = F.avg_pool2d(self.flow_cache['flow_pred'].flatten(0, 1), kernel_size=2, stride=2)  # b*f, 16, h, w
                    flow_pred_pool = torch.norm(flow_pred_pool, p=2, dim=1)  # b*f, h, w
                    super().visualize_flow_pool_batch(flow_pred_pool, batch_size=1, num_frames=3, batch_idx=0, cmap="plasma")
    ...
    def _recache_after_switch(self, output, current_start_frame, new_conditional_dict, local_start_frame=None, switch_recache_frames=None):
        ...
        # reset cross-attention cache 清空cross-attention缓存，下一步进入generator会重新计算缓存
        ...
        if switch_recache_frames is not None:  # 先前帧
            # frames_to_recache = torch.cat([switch_recache_frames, output], dim=1)[:, -21:, ...]  # 拿到最近的21帧做re cache  这里和推理不一致，推理是local_attn_size
            frames_to_recache = torch.cat([switch_recache_frames, output], dim=1)[:, -self.local_attn_size:, ...]  # 拿到最近的12帧做re cache 和推理对齐
        ...
        if dist.get_rank() == 0:
            print(f"[recache processing] num_recache_frames: {num_recache_frames}, current_start_frame: {current_start_frame}, local_start_frame: {local_start_frame}")
        ...
        self.generator.model.block_mask = block_mask  # 这个mask没有用上，直接用21帧的双向attention进行的重新计算
        with torch.no_grad():
            self.generator(
