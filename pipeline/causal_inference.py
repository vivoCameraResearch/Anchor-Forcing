    ...
    def inference(
        ...
        if self.use_prompt_cache:
            ...
            for block_index in range(self.num_transformer_blocks):
                self.prompt_cache[block_index]["switch_frame"].fill_(20)
        else:
        ...
        for n, current_num_frames in enumerate(all_num_frames):
            if profile:
                block_start.record()
            ...
            for index, current_timestep in enumerate(self.denoising_step_list):
                ...
                if index < len(self.denoising_step_list) - 1:  # 前面n段的推理
                    _, denoised_pred = self.generator(  # 这里的推理都是预测x0
    ...
    def _initialize_prompt_cache(self, batch_size, dtype, device):
        ...
        for _ in range(self.num_transformer_blocks):
            prompt_cache.append({
                ...
                "prompt_idx": torch.tensor([0], dtype=torch.long, device=device),
                "k_prior": torch.zeros([batch_size, 3 * self.frame_seq_length, 12, 128], dtype=dtype, device=device),
                "v_prior": torch.zeros([batch_size, 3 * self.frame_seq_length, 12, 128], dtype=dtype, device=device),
            })
