...
class StreamingTrainingPipeline:
    def __init__(self,
        ...
        self.rope_enhance = kwargs.get("rope_enhance", False)
        print(f"[rope_enhance policy] {self.rope_enhance}")
            ...
            for step_idx, current_timestep in enumerate(self.denoising_step_list):
                ...
                if not exit_flag:
                    ...
                    with torch.no_grad():
                        ...
                        if self.flow_cache is not None and dist.get_rank() == 0 and MY_VISUAL_DEBUG:
                            print("[SeqTrain-Pipeline] flow_pred update, flow_pred.sum is {}".format(flow_pred.sum()))
                        if self.flow_cache is not None:
                            self.flow_cache['flow_pred'] = flow_pred.detach().clone()
                    ...
                    if dist.get_rank() == 0 and MY_VISUAL_DEBUG:
                        print(f"[SeqTrain-DMD] Block {block_index} final step: enable_grad={enable_grad}, local_frame={local_start_frame}, denoise step={step_idx}")
    ...
    def _initialize_flow_cache(self, batch_size, dtype, device, video_shape, ratio):
        """
        Initialize a Per-GPU flow cache for the Wan model.
        """
        if ratio > 0:
            video_token_num = video_shape[-2] * video_shape[-1] // 4  # 在attention中下采样4倍
            self.flow_cache = {
                "flow_pred": torch.zeros([batch_size, self.num_frame_per_block, 16, video_shape[-2], video_shape[-1]], dtype=dtype, device=device), 
                "dst_token_num": int(math.ceil(video_token_num * ratio)),
                "rand_dst_token_num": int(math.ceil(video_token_num * ratio * ratio))
                }
        else:
            self.flow_cache = None
    
    def clear_flow_cache(self):
        if getattr(self, "flow_cache", None) is not None:
            self.flow_cache['flow_pred'].zero_()
    
    def _initialize_prompt_cache(self, batch_size, dtype, device):
        """
        Initialize a Per-GPU prompt cache for the Wan model.
        """
        prompt_cache = []
        for _ in range(self.num_transformer_blocks):
            prompt_cache.append({
                "k": torch.zeros([batch_size, 3 * self.frame_seq_length, 12, 128], dtype=dtype, device=device),
                "v": torch.zeros([batch_size, 3 * self.frame_seq_length, 12, 128], dtype=dtype, device=device),
                "rope_start_frame": torch.tensor([0], dtype=torch.long, device=device),
                "switch_frame": torch.tensor([1e8], dtype=torch.long, device=device),
            })
        self.prompt_cache = prompt_cache  # always store the clean cache
    
    def clear_prompt_cache(self):
        if getattr(self, "prompt_cache", None) is not None:
            for blk in self.prompt_cache:
                blk["k"].zero_()
                blk["v"].zero_()
                blk["rope_start_frame"].zero_()
                blk["switch_frame"].zero_()
