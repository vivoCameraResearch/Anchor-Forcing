...
parser.add_argument("--seed", type=int, help="Path to the config file")
...
ratios = []
# ----------------------------- Inference loop -----------------------------
for i, batch_data in tqdm(enumerate(dataloader), disable=(local_rank != 0), total=len(dataloader)):
    ...
    for seed_idx in range(config.num_samples):
        if not os.path.exists(os.path.join(config.output_folder, f'{idx}-{seed_idx}.mp4')):
            gen_list.append(seed_idx)
    if len(gen_list) == 0:
        continue
    ...
    for num in gen_list:
        ...
        video, ratio = pipeline.inference(
            noise=sampled_noise,
        ...
        output_path = os.path.join(config.output_folder, f"{idx}-{args.seed}.mp4")
        print(f'[out put path] is {output_path}')
